...
__all__ = ['data_split', 'BinDownsample', 'show_results', 'show_class_counts', 'show_durations', 'show_accuracy_history', 'show_loss_history', 'write_knn_results', 'write_hmm_results', 'write_network_results', 'write_filter_results', 'write_downsample_results', 'MoCapLoader']
...
def write_downsample_results(running_stats, name, split, number):
    for model in ('hmm', 'ffnn'):
        path = os.path.join('Experiments', 'preprocess', 'downsample', model, '{} {}'.format(name, split))
        if number is not None:
            path = path + ' ' + str(number)
        stats = running_stats[model]
        with open(path, 'w') as file:
